...
class ProductLookupParams(BaseModel):
    ...
    def validate_lookup_key(cls, values):
        if not values.get('product_id') and not values.get('item_name'):
            raise ValueError("Either product_id or item_name must be provided")
        return values
